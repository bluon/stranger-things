...
def getWord(counter):
    ...
    listCount = len(lines)
    # If we have a tweet, show that, and remove it from the list. Only display them once.
    if listCount > 0:
        word = lines[0]
    ...
    while True:
        ...
        if len(word.strip()) > 0:
            ...
            for i in range(len(ALPHABET)):
                ...
                strip.show()
                time.sleep(random.randint(5,10)/1000.0)

            time.sleep(random.randint(1,5))

            #kill all lights in a semi-random fashion
            for led in range(len(ALPHABET)):
                strip.setPixelColor(s[led]+LIGHTSHIFT, OFF)
                strip.show()
                time.sleep(random.randint(10,80)/1000.0)
